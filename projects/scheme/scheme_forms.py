...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_type(clause.first, scheme_symbolp, 0, 'let')
        if clause.first in seen_symbols:
            raise SchemeError(f'duplicate identifier: {clause.first}')
        names = Pair(clause.first, names)
        vals = Pair(scheme_eval(clause.rest.first, env), vals)
        seen_symbols.append(clause.first)
