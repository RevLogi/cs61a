...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    if num_rolls > 0:
        p = roll_dice(num_rolls, dice)
    else:
        p = boar_brawl(player_score, opponent_score)
    # END PROBLEM 3
    return p
...
def final_strategy(score, opponent_score):
    """A strategy that tries to win immediately, takes fewer risks when ahead,
    and chooses 0 dice if boar brawl (with Sus Fuss) is better than tolling 6 dice"""
    # BEGIN PROBLEM 12
    goal = GOAL
    dice = six_sided
    
    #Check if you can win by rolling 0, 1, or 2 dice
    for num_rolls in [0 , 1, 2]:
        if score + take_turn(num_rolls, score, opponent_score, dice) >= goal:
            return num_rolls
        
    #If ahead, take fewer risks
    if score > opponent_score:
        safe_rolls = 4
    else:
        safe_rolls = 6

    #Compare Boar Brawl (with Sus Fuss) to rolling best scoring dice number
    boar_score = sus_points(score + boar_brawl(score, opponent_score))
    roll_score = sus_points(score + roll_dice(num_rolls, dice))
    if boar_score - score >= roll_score - score:
        return 0
    return num_rolls
