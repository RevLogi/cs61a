class Account:
    ...
    def time_to_retire(self, amount):
        ...
        while True:
            if future_balance >= amount:
                return i
            else:
                i += 1
                future_balance = future_balance + future_balance * self.interest
...
def duplicate_link(s, val):
    ...
    if s is not Link.empty:
        if s.first == val:
            s = Link(s.first, Link(s.first, duplicate_link(s.rest, val)))
        else:
            s = Link(s.first, duplicate_link(s.rest, val))
